...
이 명령은 Git을 호출하지 않는다. 파일이 이미 있고 바이트가 같으면 반복 실행해도 변경하지 않는다. 기존 raw와 내용이 다르면 불변 규칙에 따라 덮어쓰지 않고 실패한다. 레지스트리 lock과 내용 재확인으로 동시 실행·중간 편집을 거부한다. 두 파일 복사·레지스트리 교체·사후 검증 가운데 하나라도 실패하면 이번 호출이 새로 만든 파일을 제거하고 레지스트리를 이전 내용으로 되돌린다. 레지스트리 복원 자체가 실패한 극단적 경우에는 새 레코드가 가리키는 raw 파일을 삭제하지 않고 수동 복구 경로를 오류에 남긴다.
...
- `raw:check`가 신규 파일은 현재 레지스트리와 SHA-256이 일치하는 추가로 확인하고, 기존 raw의 수정·삭제·이름 변경은 거부함
