...
새 세션에서 `하던 작업 계속 진행`이라고 입력하면 외부 원문 목록에서 실제 원문이 있는 첫 미완료 **공식 번호**를 찾고 이 문서의 기존 절차를 한 번에 하나씩 그대로 적용한다. 현재 다음 입력 대상은 공식 080 Chain-of-Thought Prompting이다. 먼저 `git status --short --branch`와 `npm run source:status -- NNN`으로 상태를 확인하고, 새 번호라면 기존 번역본을 무시한 채 `/lt NNN`부터 다시 시작한다. 완료된 단계를 중복하지 않으며 별도 브랜치·상태 파일·재개 명령은 만들지 않는다.
