...
test('renumbered source and artifact-reader URLs retain static legacy redirects', async () => {
  ...
  const fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'llm-wiki-site-redirects-'));
  const testDistDir = path.join(fixtureRoot, 'dist');
  const testEnvironment = {
    ...process.env,
    BASE_PATH: testBasePath,
    LLM_DIST_DIR: testDistDir,
  };
  ...
  try {
    ...
    const report = JSON.parse(await fs.readFile(path.join(testDistDir, 'build-report.json'), 'utf8'));
    ...
    const sourceDirectoryHtml = await fs.readFile(path.join(testDistDir, 'sources', 'index.html'), 'utf8');
    ...
    const translationsHtml = await fs.readFile(path.join(testDistDir, 'translations', 'index.html'), 'utf8');
    ...
    for (const redirect of report.redirects) {
      ...
      const redirectFile = outputFileForUrl(fromUrl, { outputDir: testDistDir, basePath: testBasePath });
    ...
    const htmlFiles = await walkFiles(testDistDir, '.html');
    ...
    await fs.rm(path.join(testDistDir, 'search-suggestions.json'));
    const negativeCheck = spawnSync(process.execPath, ['scripts/check-site.mjs'], {
      cwd: rootDir,
      env: testEnvironment,
      encoding: 'utf8',
      timeout: 120_000,
    });
    assert.equal(negativeCheck.error, undefined, negativeCheck.error?.message);
    assert.notEqual(negativeCheck.status, 0, 'check-site ignored the isolated LLM_DIST_DIR fixture.');
    assert.match(
      `${negativeCheck.stderr}\n${negativeCheck.stdout}`,
