...
export function buildKnowledgeGraph(documents, {
  ...
  const nodes = published.map((document) => ({
    ...
    evidenceCoverage: document.evidenceCoverage ?? document.verification,
    contentMode: document.contentMode ?? 'descriptive',
    editorialStatus: document.editorialStatus ?? document.lifecycle ?? 'active',
