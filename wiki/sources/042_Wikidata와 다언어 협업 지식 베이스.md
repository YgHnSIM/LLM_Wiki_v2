...
Item은 언어와 표시 이름이 달라도 같은 대상을 가리키는 Q ID를 갖고, property는 관계와 값의 종류를 나타내는 P ID를 갖는다. 사용자는 item에 property–value statement를 붙이고, 필요하면 그 값의 범위와 출처 정보를 함께 기록한다.
...
1. Wikidata의 item·property·label·sitelink는 각각 어떤 역할을 하는가?
2. statement가 단순한 주어–속성–목적어 triple보다 더 많은 정보를 보존하는 방식은 무엇인가?
3. 다언어 식별자와 공동 편집·SPARQL 서비스가 정확성이나 무제한 사용을 자동 보장하지 않는 이유는 무엇인가?
