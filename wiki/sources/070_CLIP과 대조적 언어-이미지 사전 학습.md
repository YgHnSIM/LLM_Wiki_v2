...
연구진은 공개적으로 접근 가능한 여러 인터넷 출처에서 이미지-텍스트 쌍 4억 개를 수집하고 이를 WebImageText(WIT)라고 불렀다. 시각 개념의 범위를 넓히기 위해 텍스트가 약 50만 개 검색 질의 중 하나를 포함하는 쌍을 찾았고, 결과를 대략적으로 class balance하기 위해 질의당 최대 2만 쌍을 포함했다.
