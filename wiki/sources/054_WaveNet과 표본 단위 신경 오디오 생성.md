...
learning:
  ...
  outcomes:
    - WaveNet이 앞선 오디오 표본에서 다음 표본의 분포를 만들고 팽창 인과 합성곱으로 문맥을 넓힌 방식을 설명할 수 있다.
  next:
    - target: concept.wavenet
      reason: 'WaveNet — 표본별 확률모형, 조건화, 생성 속도의 trade-off를 개념 단위로 다시 정리한다.'
---
...
> **읽고 나면:** WaveNet이 앞선 오디오 표본에서 다음 표본의 분포를 만들고 팽창 인과 합성곱으로 문맥을 넓힌 방식을 설명할 수 있다.

## 1단계 — 먼저 잡을 핵심

원문은 2016년 [[WaveNet]]을 원시 파형 생성의 혁명으로 설명한다. 직접 파형·팽창 인과 합성곱·자기회귀 생성의 중요성은 잘 짚지만, 자연 음성과의 실제 평가 격차, 외부 언어 특징 조건화, 2016년 연구 원형과 2017년 제품 배포를 섞는다. 공개 문서는 모델의 확률 단위와 제품 전환의 시점을 나누어 검증한다.
...
앞에서 생성한 표본들을 인과 합성곱으로 읽어 다음 표본의 확률분포를 만든 뒤, 그 분포에서 한 값을 선택한다. 선택한 값은 곧 다음 예측의 입력이 되므로 훈련 위치는 병렬로 계산할 수 있어도 실제 생성은 한 표본씩 진행된다.
...
팽창은 수용 영역을 수천 표본으로 키웠지만 원형이 직접 다루는 문맥은 16kHz에서 수백 밀리초 규모였다. 국소 음향과 중간 범위 운율을 포착하는 것과 수초 전 사건이나 문장·음악 전체 구조를 직접 모델링하는 것은 같은 주장이 아니다.
...
WaveNet은 neural vocoder, WaveRNN, flow·diffusion 기반 파형 생성과 신경 코덱 오디오 모델이 품질·속도·표현 해상도를 다시 설계하도록 한 중요한 기준점이다. 팽창 인과 합성곱도 시간 합성곱 네트워크와 다양한 시퀀스 모델에 널리 쓰였다.

하지만 모든 현대 생성 오디오가 원시 파형을 직접 생성하지는 않는다. 스펙트로그램, latent, codec token을 생성한 뒤 별도 디코더로 파형을 만드는 구조도 많다. WaveNet이 연속형 양식의 신경 생성을 가능하게 했다는 사실과 이미지·비디오 생성이 WaveNet에서 직접 나왔다는 계보는 구분한다.

원문이 MERT와 WaveNet을 ‘최종 결과 직접 최적화’로 묶은 부분도 부정확하다. MERT는 BLEU 같은 번역 지표를 직접 최적화하는 반면, 원 WaveNet은 다음 양자화 표본의 로그가능도를 최대화했다. WaveNet이 MOS나 인간 지각 품질을 직접 목적함수로 학습한 것은 아니다.
...
1. WaveNet은 원시 오디오의 다음 표본을 어떤 형태의 출력으로 예측하는가?
2. 팽창 인과 합성곱은 문맥 범위와 미래 정보 차단을 어떻게 함께 구현하는가?
3. 원 논문의 음성 합성과 2017년 제품 배포를 같은 시스템으로 볼 수 없는 이유는 무엇인가?

### 다음 문서

- [[concept.wavenet|WaveNet]] — 표본별 확률모형, 조건화, 생성 속도의 trade-off를 개념 단위로 다시 정리한다.
