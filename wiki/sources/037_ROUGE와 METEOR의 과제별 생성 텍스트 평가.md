...
원문은 2004년에 [[ROUGE]]와 [[METEOR]]가 함께 등장해 [[BLEU]]의 정밀도 중심·문자열 일치 중심 평가를 과제 특화 재현율과 의미 지식으로 보완했다고 설명한다. 공개 문서는 **2004년 ROUGE 논문**, **2004년 번역 평가 recall·stemming 선행 연구**, **2005년 정식 METEOR 논문**을 분리하고 각 지표의 실제 계산과 인간 평가 상관 범위를 복원한다.

핵심 통찰은 하나의 지표가 모든 생성 과제의 품질을 평가하지는 않는다는 것이다. 그러나 ROUGE가 “중요 정보”를 원문에서 직접 알아보거나 METEOR가 문맥 의미를 이해하는 것도 아니다. 두 지표 모두 후보와 사람이 만든 참조 사이의 관측 가능한 대응을 세며, 참조 구성·전처리·길이·언어 자원과 집계 단위에 조건부다.
...
먼저 후보와 참조를 같은 방식으로 토큰화한다. ROUGE는 참조에 있는 n-gram·부분수열을 후보가 얼마나 되찾았는지 세고, METEOR는 단어를 일대일로 맞춘 뒤 precision·recall과 어순 단편화를 결합한다. 마지막에는 과제·길이·참조 수가 같은 조건에서 시스템 점수와 인간 판단을 비교한다.
...
1. ROUGE-N과 METEOR는 후보와 참조의 대응을 각각 어떤 단위와 정규화로 계산하는가?
2. METEOR의 exact·stem·WordNet 정렬과 chunk 벌점은 어떤 순서로 점수를 만드는가?
3. 높은 시스템 수준 상관이 문장별 인간 판단이나 다른 언어·과제에도 그대로 일반화되지 않는 이유는 무엇인가?
