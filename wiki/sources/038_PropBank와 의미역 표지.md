...
원문은 PropBank를 2005년에 시작된 최초의 대규모 의미 주석 자원으로 소개하고, 번호형 논항을 보편 의미역처럼 설명하며 정보 추출·질의응답·기계 번역·AMR·신경 언어 모델까지 하나의 영향사로 연결한다. 이 공개 문서는 **2005년 대표 논문과 더 이른 프로젝트 단계**, **술어별 역할과 전역 역할의 차이**, **주석 자원과 자동 [[의미역 표지]] 과제**, **직접 확인된 후속 제도와 가능한 응용**을 분리한다.
...
먼저 문장에서 술어의 뜻에 맞는 roleset을 고르고, Penn Treebank 구문 노드 가운데 각 참여자와 수식어에 해당하는 범위를 찾아 역할을 붙인다. 이렇게 쌓은 주석으로 새 문장의 논항 경계와 역할을 예측하는 모델을 학습하고, 정확한 구문과 자동 구문 조건을 나누어 평가한다.
...
1. PropBank의 roleset과 번호형 논항은 문장 의미 전체가 아니라 무엇을 주석하는가?
2. Penn Treebank 구문 노드에 역할을 붙인 자료가 CoNLL의 자동 의미역 표지 과제로 이어지는 흐름은 어떻게 설명할 수 있는가?
3. Arg2 이상의 번호나 2005년 시점을 모든 술어·언어에 일반화하면 안 되는 이유는 무엇인가?
