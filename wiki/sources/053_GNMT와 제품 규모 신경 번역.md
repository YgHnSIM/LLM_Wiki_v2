...
원문은 2016년 Google Neural Machine Translation(GNMT)을 신경 번역의 제품 규모 전환으로 설명한다. 큰 방향은 맞지만, 단일한 “종단간 신경망”이라는 표현만으로는 8층 LSTM·어텐션·잔차 연결·WordPiece·탐색 보정·분산 실행을 결합한 실제 시스템이 보이지 않는다. 공개 문서는 초기 seq2seq와 GNMT의 차이, 논문 평가와 최초 제품 배포 범위를 나누어 검증한다.
