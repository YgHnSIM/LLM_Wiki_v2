...
논문은 accordion으로 만든 tapir, sweater를 입은 hedgehog, 여러 대상의 색·위치 지정 같은 예시를 제시했다. 동시에 변수 결속(variable binding)이 일관되지 않고, 대상 수를 늘리거나 같은 뜻의 문구를 다르게 쓰면 결과가 불안정해진다고 밝혔다. 저자들이 관찰한 제한적인 조합 행동과 일반적인 구성적 이해를 구분한다.
