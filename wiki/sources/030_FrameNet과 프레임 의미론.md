...
> **난이도:** 입문<br>
> **선수 지식:** 없음<br>
> **읽고 나면:** FrameNet의 프레임·프레임 요소·어휘 단위·주석 층을 설명하고, 자원 자체와 자동 의미 분석 모델을 구분할 수 있다.
...
어휘 단위는 특정 문맥에서 프레임을 환기하고, 프레임 요소는 그 프레임 안의 참여자와 속성을 구분한다. 말뭉치 주석은 실제 문장에서 이 요소가 어떤 구문 형식으로 실현되는지 연결하며, 자동 분석 모델은 이 자원을 별도의 훈련·평가 자료로 사용한다.
...
| 시기 | 확인되는 사건 | 해석의 경계 |
| --- | --- | --- |
| 1960년대 후반 | Fillmore가 Case Grammar에서 논항의 의미 역할과 통사 실현 문제를 다루었다. | FrameNet 데이터베이스가 이미 존재한 시기가 아니다. |
...
1. FrameNet의 프레임, 프레임 요소, 어휘 단위와 말뭉치 주석은 어떻게 연결되는가?
2. `Commerce_buy` 예시에서 어휘 단위 목록을 실제 버전 자료로 확인해야 하는 이유는 무엇인가?
3. FrameNet 자원이 존재한다는 사실만으로 자동 의미 분석 전체가 해결됐다고 말할 수 없는 이유는 무엇인가?
