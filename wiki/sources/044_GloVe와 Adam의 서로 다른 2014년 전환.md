...
원문은 [[GloVe]]와 [[Adam 최적화기]]를 “숲과 나무를 함께 보게 한” 2014년의 한 쌍의 혁신으로 묶는다. 그러나 둘은 공동 연구도, 같은 문제에 대한 경쟁 해법도 아니다. GloVe는 말뭉치의 단어–문맥 동시출현 계수에서 정적 [[단어 임베딩]]을 학습하고, Adam은 확률적 목적의 그래디언트로 매개변수를 갱신한다. 공개 문서는 **공통 연대와 직접 기술 계보를 분리**해 두 논문을 각각 검증한다.
...
GloVe는 말뭉치에서 만든 동시출현 통계를 단어 벡터로 근사하고, Adam은 목적 함수의 그래디언트 이력을 이용해 매개변수 갱신 크기를 조절한다. 두 방법은 각각 표현 학습과 최적화라는 서로 다른 층의 문제를 다룬다.
...
1. GloVe에서 전역 통계는 어떤 국소 사건을 말뭉치 전체에 집계한 것인가?
2. Adam은 그래디언트의 어떤 두 이동 평균을 사용해 매개변수 갱신을 조절하는가?
3. GloVe와 Adam이 같은 해에 제안됐다는 사실만으로 두 연구를 하나의 결합된 기술 전환으로 볼 수 없는 이유는 무엇인가?
