...
쉽게 말하면 ADALINE 하나가 입력에 대해 하나의 이진 판단을 내리고, MADALINE I은 이런 여러 판단을 정해진 논리로 합친다. 여러 학습 요소와 결합 규칙을 한 시스템에 넣었다는 점이 중요하지만, 각 층을 모두 같은 방식으로 학습하는 현대 다층 신경망과는 구분해야 한다.
