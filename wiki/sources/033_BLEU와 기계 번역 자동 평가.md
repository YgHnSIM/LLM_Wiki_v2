...
원문은 BLEU(Bilingual Evaluation Understudy)를 느린 인간 평가를 대신해 기계 번역 연구를 가속한 자동 지표로 설명한다. 공개 문서는 Papineni·Roukos·Ward·Zhu의 2002년 원 논문에서 실제 정의와 실험 범위를 복원한다. BLEU의 핵심은 후보 번역과 여러 참조 번역 사이의 **수정 n-gram 정밀도**, 짧은 출력에 대한 **brevity penalty**, 여러 차수 정밀도의 **가중 기하평균**, 그리고 **말뭉치 단위 집계**다.
...
먼저 후보의 n-gram 반복 횟수를 각 참조가 허용하는 최대 횟수까지만 센다. 다음으로 unigram부터 더 긴 n-gram까지의 수정 정밀도를 결합해 단어 선택과 국소 어순을 함께 본다. 마지막으로 후보가 유효 참조 길이보다 짧으면 brevity penalty를 곱한다.

이를 최소 식으로 쓰면 $\operatorname{BLEU}=\operatorname{BP}\times G(p_1,\ldots,p_N)$다. $G$는 여러 차수 수정 정밀도의 가중 기하평균이고, $\operatorname{BP}$는 지나치게 짧은 번역에 주는 벌점이다.
...
1. BLEU는 후보 번역의 n-gram 일치를 어떤 방식으로 clip해 집계하는가?
2. 여러 차수 정밀도와 brevity penalty는 각각 어떤 단순한 점수 악용을 막는가?
3. BLEU 점수를 인간 품질 판단이나 미분 가능한 학습 손실과 같게 볼 수 없는 이유는 무엇인가?
