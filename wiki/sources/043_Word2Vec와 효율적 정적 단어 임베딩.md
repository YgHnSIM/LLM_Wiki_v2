...
원문은 2013년 [[Word2Vec]]이 처음으로 단어의 의미를 수치에 담고, 얕은 신경망이 인간처럼 언어를 이해하게 했다고 설명한다. 공개 문서는 Mikolov 등의 세 논문과 후속 분석을 기준으로 **밀집 표현의 선행 역사**, [[CBOW]]·[[Skip-gram]]의 실제 예측 구조, 계층적 softmax와 부정 샘플링의 도입 시기, 벡터 유추 평가의 범위를 복원한다.
...
1. Word2Vec가 하나의 고정 모델이 아니라 방법군이라는 말은 어떤 선택들을 가리키는가?
2. CBOW와 Skip-gram은 문맥과 중심 단어 가운데 무엇을 입력과 예측 대상으로 삼는가?
3. 벡터 유추 성과가 단어 의미의 완전한 이해나 현대 문맥 임베딩과의 동일성을 보장하지 않는 이유는 무엇인가?
