...
아홉 본편은 시간의 앞뒤를 잡고, 세 연결고리는 여러 장 사이에서 생략되기 쉬운 **변환 과정**을 가로질러 설명한다. 본편을 새로 늘리는 대신 같은 사건을 표현 workload, software abstraction, silicon-to-system이라는 세 축으로 다시 읽는다.

| 연결고리 | 가로지르는 본편 | 채우는 질문 |
| --- | --- | --- |
| [[N-gram에서 LLM으로]] | 4 → 5 → 6 | Sparse count·Katz back-off가 어떻게 NPLM·Word2Vec·RNN·seq2seq·Transformer의 학습 표현과 조건부 생성 workload로 바뀌었는가? |
| [[저장 프로그램에서 학습 프레임워크까지]] | 2 → 5 → 6 | Stored instruction 뒤에 Fortran·BLAS·역모드 자동미분·CUDA·cuDNN·TensorFlow의 어떤 추상화와 계약이 쌓였는가? |
| [[집적도 증가에서 가속기까지 무엇이 더 필요했나]] | 2 → 5 → 8 | Moore의 집적도 관찰에서 실제 GPU·TPU 성능까지 power·memory·parallel programming·domain specialization이 왜 필요했는가? |

가장 짧게는 **4장 → 표현 연결고리 → 5장 → software·silicon 연결고리 → 6장·8장** 순서로 읽는다. 각 연결고리도 본편과 같은 여섯 항목 측정 장부와 네 종류 인과 표지를 사용하므로, “A가 B를 낳았다”가 아니라 무엇이 직접 영향이고 무엇이 가능 조건인지 대조할 수 있다.
...
Corpus가 커졌다는 말에서 출발해 quality proxy·분포·sampling·반복 token을 나누고, model–data–compute 배분에서 어떤 data 조건을 고정해야 하는지 확인한다.

### 측정과 평가 축을 보고 싶다면

[[기계 이전의 계산은 어떻게 능력이 되었나]] → [[자동 평가 지표는 무엇을 보상하는가]] → [[모델 능력에서 서비스 능력으로]]

검산과 결과 계약에서 출발해 metric의 분모·proxy·집계 유인을 확인하고, service의 품질·tail latency·가용성 계약으로 측정 경계를 넓힌다.
