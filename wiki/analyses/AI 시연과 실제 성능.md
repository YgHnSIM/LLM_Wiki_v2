...
[[AI 시연과 실제 성능]]의 핵심 쟁점은 통제된 조건에서 성공한 AI 시스템을 실제 환경에서도 같은 수준으로 작동한다고 해석할 수 있는가에 있다. [[003_Georgetown-IBM 기계 번역 시연]]부터 [[040_IBM Watson과 Jeopardy 질의응답]], [[081_ChatGPT 연구 미리보기와 대화형 LLM 배포]]까지 공개 시연은 실제 기술적 성과와 설계된 공개 조건을 동시에 가진다. 중요한 것은 시연을 가짜로 치부하거나 일반 지능의 증명으로 확대하는 양극단이 아니라, **무엇을 입력받았고, 어떤 조건에서 측정됐으며, 관객이 본 결과가 어느 능력을 대표하는지**를 분리하는 일이다.
...
| 층 | 묻는 질문 | 각 사례에서 확인할 것 |
| --- | --- | --- |
| 인터페이스 | 시스템이 실제로 무엇을 입력받고 무엇을 출력했는가? | 선별 문장, 텍스트 대화, 블록 명령, 전자 clue와 buzzer, ChatGPT 다중 턴 UI |
| 과업 경계 | 어떤 어휘·자료·규칙·감각 양식과 제품 층이 포함되거나 제외됐는가? | 약 250개 어휘, DOCTOR 역할, 블록 세계, Watson의 로컬 corpus와 A/V 제외, ChatGPT의 모델·RLHF·UI·filter·feedback 경로 |
| 측정 | 성공을 어떤 표본과 지표로 계산했는가? | 60문장 시연, 대화 인상, 기능 예제, blind clue 정밀도·답변률·시간, ChatGPT 발표의 선택된 대화 예시와 보고 예정 feedback |
| 공개 효과 | 관객이 본 승리·유창성·행동이 무엇을 더 넓게 암시했는가? | 자동 번역 기대, 공감과 언어 이해의 인상, 게임 우승과 일반 지능, 유창한 대화와 일반 정확성·안전성의 혼동 |

이 네 층을 나누면 시연이 특정 조건에서 이루어졌다는 이유로 실제 공학 성과를 지우지 않으면서, 공개 장면이 측정하지 않은 능력까지 일반화하는 오류도 피할 수 있다. 특히 인터페이스가 인간에게 보이는 환경을 기계가 직접 지각한 것처럼 보이게 할 때에는 입력 경로를 별도로 확인해야 한다.

### ChatGPT 연구 미리보기의 대화 예시와 제품 층

OpenAI의 2022년 ChatGPT 발표는 후속 질문, 오류 인정, 잘못된 전제 반박과 요청 거절을 설명하고 여러 대화 예시를 제시했다. 이 예시는 대화 형식이 어떤 상호작용을 목표로 했는지 보여 주는 **선택된 출시 사례**다. 발표가 함께 밝힌 사용자 문제 보고와 content filter의 오탐·미탐 feedback 계획은 실제 배포에서 실패를 찾는 관측 경로다. 그러나 어느 쪽도 대표 표본에서 정확도·위해율·calibration을 계산한 독립 benchmark 결과는 아니다.

따라서 당시 ChatGPT를 평가할 때에는 적어도 다음 다섯 층을 나눈다.

1. **기반 모델:** 2022년 초 훈련을 마친 GPT-3.5 series의 한 모델이다. 공식 발표는 초기 ChatGPT의 정확한 매개변수 수를 공개하지 않았으므로 GPT-3·InstructGPT의 175B를 소급하지 않는다.
2. **대화형 RLHF:** 인간 trainer의 대화 시연으로 SFT하고, 여러 응답의 순위로 reward model을 학습한 뒤 PPO로 정책을 갱신했다.
3. **다중 턴 UI:** 사용자가 앞선 답을 이어 질문·정정할 수 있게 했다. 이는 session·계정을 넘어서는 persistent memory의 증거가 아니다.
4. **Moderation·외부 filter:** 특정 unsafe content를 경고·차단하는 층이 있었지만 공식 발표 자체가 false positive와 false negative를 한계로 적었다.
5. **Feedback 경로:** 문제 출력과 filter 오류를 UI로 수집해 후속 개선에 쓰는 iterative deployment 통로다. 피드백을 받는다는 사실은 안전성이나 대표 성능의 입증이 아니다.

이 분해는 유창한 답 하나가 기반 모델만의 산출인지, 대화 학습·인터페이스·운영 완화책까지 합친 제품 행동인지 묻게 한다. 또한 여기서 말하는 `ChatGPT`는 2022년 무료 연구 미리보기다. 후대의 모델, 검색·도구, multimodal 입력, memory와 현재 정책을 당시 시연에 소급하지 않는다.
...
각 사례에서 확인되는 입력 경로, 과업 조건, 반복 평가와 공개 결과는 개별 1차 자료와 기술 기록에 근거한다. Georgetown의 선별 문장, SHRDLU의 닫힌 세계, Watson의 전자 clue와 로컬 자료 조건은 서로 다른 경계다. ChatGPT의 공식 발표는 대화형 RLHF·다중 턴 UI·filter·feedback 경로와 알려진 한계를 공개했지만, 선택된 대화 예시를 대표 benchmark로 제시하지 않았다.
...
AI 역사에서 반복되는 교훈은 세 가지다. 첫째, 공개 장면보다 먼저 시스템이 실제로 받은 입력과 결과에 관여한 제품 층을 확인한다. 둘째, 시연 결과와 별도의 반복 가능한 평가를 찾는다. 셋째, 새 영역·새 버전으로의 전이는 자료·인터페이스·오류 비용이 달라지는 별도 주장으로 취급한다. 이 원칙은 1954년 기계 번역, ELIZA의 대화 인상, SHRDLU의 블록 세계, Watson의 게임 우승과 2022년 ChatGPT 연구 미리보기를 같은 잣대로 평탄화하지 않으면서도 비교할 수 있게 한다.

### 비교를 통한 해석

인터페이스·과업 경계·측정·공개 효과의 네 층은 여러 사례를 함께 읽기 위한 합성 틀이다. ChatGPT에는 이 틀 안에서 기반 모델·대화형 RLHF·다중 턴 UI·moderation·feedback 경로를 다시 분해했다. 이 틀은 좁은 조건의 성공을 지우지 않으면서 조건 밖의 능력이나 현재 제품으로 확대하는 해석을 막는다.

### 아직 입증되지 않은 계보

초기 공개 시연들이 현대 LLM으로 직접 이어졌거나, 어느 한 시연의 성공이 경계 없는 일반 지능을 입증했다는 계보는 이 자료로 확인되지 않는다. 2022년 발표만으로 ChatGPT의 현재 제품 기능이나 장기 사회 효과까지 설명하는 계보도 입증되지 않는다.

## 학습 확인

### 확인 질문

1. 공개 AI 시연을 평가할 때 분리해야 하는 네 층은 무엇이며, 선택된 예시와 대표 benchmark는 어떻게 다른가?
2. Watson의 경기 우승과 원시 질의응답 성능 평가는 왜 같은 측정이 아닌가?
3. 2022년 ChatGPT를 다섯 제품 층으로 나누는 이유와 공식 발표만으로 175B·현재 제품 기능을 주장할 수 없는 이유는 무엇인가?
...
- OpenAI, [Introducing ChatGPT](https://openai.com/index/chatgpt/), 2022-11-30; 도입부와 `Methods`·`Limitations`·`Iterative deployment`.
