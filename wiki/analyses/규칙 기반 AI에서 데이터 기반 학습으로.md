...
각 시스템에서 사람이 직접 정한 규칙·표현·구조와 데이터에서 추정한 값을 먼저 나눈다. 이어 새 영역으로 옮길 때 늘어나는 수작업과 학습 가능한 범위를 비교하면, 전환을 완전 교체가 아니라 설계 경계의 이동으로 읽을 수 있다.
...
데이터 기반 전환을 인간 설계의 제거가 아니라 설계 대상과 학습 가능 범위의 이동으로 보는 결론은 여러 과제의 자료를 함께 읽은 합성 해석이다.

### 아직 입증되지 않은 계보

ATN·몬태규 의미론·Lesk·HMM·TDNN·CNN 가운데 어느 하나가 현대 LLM의 단일 직접 조상이라는 계보는 등록된 근거가 입증하지 않는다. 공통점은 설계와 학습의 경계가 반복해서 재배치됐다는 데 있다.
...
2. HMM, Penn Treebank, TDNN과 CNN은 수작업 구조와 학습을 각각 어떻게 결합했는가?
