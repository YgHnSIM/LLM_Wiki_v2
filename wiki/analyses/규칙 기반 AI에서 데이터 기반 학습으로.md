...
같은 음성 인식 과제에서도 학습 경계는 한 번에 이동하지 않았다. [[041_심층 신경망 음향 모델과 DNN-HMM 전환]]은 기존 파이프라인의 음향 모델을 깊은 신경망으로 바꾼 사례이고, [[087_Whisper와 대규모 약한 감독 음성 인식]]은 음향 encoder와 text decoder를 함께 학습해 언어·과제·시간을 token 출력으로 통합한 사례다. 둘을 함께 읽으면 “신경망을 썼는가”보다 **어느 구성요소까지 학습했고 무엇을 설계·평가 절차로 남겼는가**가 더 유용한 비교 질문이 된다.

## 2단계 — 작동 원리

### 비교 논리

각 시스템에서 사람이 직접 정한 규칙·표현·구조와 데이터에서 추정한 값을 먼저 나눈다. 이어 새 영역으로 옮길 때 늘어나는 수작업과 학습 가능한 범위를 비교하면, 전환을 완전 교체가 아니라 설계 경계의 이동으로 읽을 수 있다. 음성 인식에서는 입력 특징, 상태·라벨, 발음 사전과 언어 모델, decoder, 평가 normalizer와 장문 추론 규칙을 별도 축으로 놓아야 학습 범위가 넓어진 지점과 고정 설계가 이동한 지점을 구분할 수 있다.
...
이 시간표에는 수작업 규칙과 학습된 통계 모델만으로 나뉘지 않는 별도의 중간 계열도 있다. [[017_Lesk 알고리즘과 단어 의미 중의성 해소]]의 [[Lesk 알고리즘]]은 1986년 기계 판독형 사전을 사용해 [[단어 의미 중의성 해소]]를 수행했다. 후보 의미의 정의와 문맥 단어들의 정의 사이에서 정확히 일치하는 어휘를 세어 최대 중첩을 고르는 결정론적 지식 기반 절차였으며, 자료에서 매개변수나 확률을 학습한 통계 모델은 아니었다.
...
[[025_WordNet과 어휘 의미망]]의 [[WordNet]]은 이 중간 계열을 자원 구축의 층위에서 더 선명하게 보여 준다. 사람이 synset과 어휘·의미 관계를 편찬하고, 후속 알고리즘은 그 구조를 탐색하거나 점수화했다. 이는 과제별 규칙을 전부 코드로 쓰는 방식도, 말뭉치에서 표현을 자동 학습하는 방식도 아니며, 사람이 만든 표현 자원과 계산 절차를 결합한 접근이다.
...
통계적 자연어 처리는 구조를 없앤 것이 아니다. HMM 태거는 품사를 상태로 사용하고, 확률 문법은 형식 문법의 규칙에 가중치를 둔다. Brill의 1992년 태거는 주석 [[말뭉치 기반 학습|말뭉치]]에서 사람이 읽을 수 있는 변환 규칙을 학습했다. 이 사례들은 상징 표현·수작업 설계·통계 추정이 하나의 시스템 안에 공존할 수 있음을 보여 준다.
...
### DNN-HMM에서 Whisper까지: 학습 경계의 확장

2010–2012년의 [[DNN-HMM]]은 HMM 전체를 신경망으로 대체하지 않았다. DNN은 고정 음향 특징 프레임에서 문맥 의존 HMM 상태인 senone의 사후확률을 예측하며 GMM 음향 모델을 교체했다. 초기 학습 표적과 프레임 정렬은 기존 HMM 계열 시스템에서 얻었고, HMM 상태 전이·발음 사전·별도 언어 모델·단어열 탐색 decoder는 계속 사용했다. 따라서 학습 가능한 표현은 음향 모듈 안에서 크게 넓어졌지만 파이프라인의 다른 경계는 유지됐다.

[[Whisper]]는 30초 음향 표현을 받는 Transformer encoder와 자기회귀 text decoder를 약 68만 시간의 웹 오디오-전사 쌍으로 함께 학습했다. 별도 발음 사전·외부 언어 모델·HMM 상태열을 거치지 않고 byte-level BPE text를 생성하며, 언어·원언어 전사/X→English 번역·timestamp·`nospeech`를 특수 token 형식으로 통합했다. 이 변화는 신경망이 담당하는 범위를 음향 상태 추정에서 여러 음성 과제의 token sequence 생성까지 넓혔다.

그러나 학습 범위의 확장이 인간 설계의 소멸은 아니다.

| 단계 | 데이터에서 학습되는 부분 | 사람이 계속 정하는 부분 |
| --- | --- | --- |
| HMM 파이프라인 | 상태 전이·관측 확률과 언어 모델 확률 | 음향 특징, 상태·발음 단위, 발음 사전, 모델 분해와 탐색 절차 |
| DNN-HMM | 고정 프레임 특징에서 senone 사후확률을 예측하는 깊은 음향 모델 | 입력 특징, HMM 상태·정렬·묶음, 발음 사전, 외부 언어 모델과 decoder |
| Whisper | 음향 encoder와 자기회귀 text decoder, 언어·과제·시간·무음 token의 조건부 분포 | 16 kHz·30초·80채널 log-Mel 입력, 합성곱 전처리, BPE와 task-token 문법, 자료 필터, 평가 normalizer와 장문 decoding heuristic |

Whisper의 고정 설계는 단순한 주변 장식이 아니다. 30초보다 긴 오디오는 timestamp로 창을 이동하고 이전 text 조건, beam search, temperature fallback, 무음·로그확률 threshold와 초기 timestamp 제약을 조합해 처리한다. 평가 WER도 광범위한 text normalizer에 따라 크게 달라진다. 그러므로 “데이터와 Transformer가 전처리·출력 규칙·평가·추론을 모두 자동 발견했다”고 결론 내릴 수 없다.

이 비교는 같은 ASR 과제에서 학습 가능한 구성요소의 범위가 이동한 모습을 보여 준다. [[시간 지연 신경망]]·DNN-HMM·Whisper가 하나의 직선적 발명 계보라는 뜻은 아니다. Whisper 논문은 범용 encoder–decoder Transformer를 채택했으며, 등록된 근거는 1989년 TDNN이나 2012년 DNN-HMM을 그 구조의 직접 선조로 제시하지 않는다.
...
각 문헌은 규칙, 자원, 주석, 확률 추정, 학습 가능한 가중치가 서로 다른 방식으로 결합됐음을 보여 준다. NLP의 통계적 전환과 신경망 적용도 하나의 연도나 단일 발명으로 합쳐지지 않는다.
...
데이터 기반 전환을 인간 설계의 제거가 아니라 설계 대상과 학습 가능 범위의 이동으로 보는 결론은 여러 과제의 자료를 함께 읽은 합성 해석이다. HMM에서 DNN-HMM과 Whisper로 갈수록 공동 학습되는 범위는 넓어졌지만, 설계 책임은 입력 특징·자료 선별·token interface·평가 normalizer·장문 decoding으로 이동했다.

### 아직 입증되지 않은 계보

ATN·몬태규 의미론·Lesk·HMM·TDNN·CNN 가운데 어느 하나가 현대 LLM의 단일 직접 조상이라는 계보는 등록된 근거가 입증하지 않는다. HMM→DNN-HMM→Whisper도 같은 과제 안의 설계 경계를 비교하는 분석 축이지, 각 구조가 다음 구조를 직접 낳았다는 단일 발명 계보가 아니다. 공통점은 설계와 학습의 경계가 반복해서 재배치됐다는 데 있다.

## 학습 확인

### 확인 질문

1. 규칙 기반 접근의 지식 공학 병목은 데이터 기반 시스템에서 어떤 설계 문제로 옮겨 가는가?
2. HMM·DNN-HMM·Whisper에서는 각각 무엇을 데이터에서 학습하고, 어떤 입력·상태·token·평가·추론 설계를 고정했는가?
3. AI 역사를 규칙의 실패와 데이터의 승리로만 정리하면 어떤 시간표와 중간 계열을 놓치는가?
...
- Alec Radford 외, [Robust Speech Recognition via Large-Scale Weak Supervision](https://arxiv.org/abs/2212.04356), 2022, §§2.1–2.4·3.1–3.9·4.2–4.5와 Figure 1.
