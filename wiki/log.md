...
## [2026-07-26] site | Refine reading, navigation, and search controls

변경 내용:

- 기존 Lo-Fi 인쇄물 톤을 유지한 채 작은 화면의 긴 제목·breadcrumb·문서 사실표를 재배치했다. 제목은 의미 단위로 줄바꿈되고, 대표 근거는 생략되지 않으며, 위로 가기 동작은 본문을 가리지 않는다.
- 모바일 메뉴를 native dialog로 바꾸어 열기·닫기·Esc·backdrop·초점 복귀를 일관되게 처리했다. JavaScript가 없을 때의 독립 메뉴와 native dialog 미지원 fallback도 남겼다. 빠른 검색은 제목·별칭·태그 후보를 키보드로 탐색할 수 있고, 전체 검색은 본문까지 포함해 결과 페이지로 이어진다.
- 전체 검색과 목록에 검증 상태·근거·연결 수를 기준으로 한 정렬 피드백, 현재 결과 수, 필터 초기화와 모바일의 query-first 상세 필터 흐름을 추가했다. 원문 노트의 대표 근거 표기는 kind 내부값 대신 저자·연도로 읽히게 정리했다.
- 홈의 새 학습 경로 CTA를 진단 시작 위치로 연결하고, 중복된 원문 목록 선택지·반복된 개념 진입 동작을 정리했다. 읽기 전환 버튼은 현재 자료의 차이를 더 짧고 명확하게 드러낸다.

검증 결과:

- `npm run verify`, native dialog fallback 회귀를 추가한 뒤의 `npm run test:browser`, `git diff --check`를 통과했다. Node 회귀 검사와 Playwright Chromium 상호작용 검사는 JavaScript 없음·native dialog fallback을 포함한 모바일 메뉴, autocomplete, 검색 필터, 목록 정렬, 긴 제목을 함께 확인한다.
- Playwright Chromium에서 홈, 모바일 메뉴, 전체 검색, 원문 노트 목록, 긴 원문 노트를 desktop과 390×844 viewport로 시각 검수했다. page-level 가로 overflow와 console error가 없었다.

남은 제한:

- 자동완성 후보는 로컬 정적 색인에 기반하며, 본문 전체 검색의 순위·문맥 판정은 결과 페이지에서 수행한다. 메뉴와 필터의 상태는 서버 계정이나 기기 사이에 동기화하지 않는다.

## [2026-07-26] site | Balance desktop directory cards

변경 내용:

- desktop 1081px 이상에서 원문 노트·개념·인물·비교 읽기 디렉터리의 간략·카드 보기를 두 열 카탈로그로 맞췄다. 검색 결과는 이미 두 열이므로 변경하지 않았고, 모바일은 한 열로 유지했다.
- 원문 노트의 상세 카드 보기에서는 세로 타임라인 선을 제거하고 번호 영역을 작은 리소그래프 스탬프로 줄였다. 두 카드가 나란히 놓여도 제목·요약·태그가 읽히는 폭을 확보했다.
- 홈의 ‘첫 번째 자료부터 읽기’ 타임라인은 순차 독해의 의미를 보존하도록 한 열로 남겼다.

검증 결과:

- `npm run verify`, `git diff --check`를 통과했다. Playwright Chromium 회귀 검사는 desktop 디렉터리의 두 열과 모바일 가로 overflow를 확인한다.
- 원문 노트의 간략·상세 카드, 개념의 간략·상세 카드, 390×844 모바일 목록을 시각 검수했다.

남은 제한:

- 이 배치는 카드 폭과 탐색 밀도만 조정한다. 별도 카드 콘텐츠의 요약 길이·태그 수를 재편집하지 않았으며, 시간순 정렬은 DOM 순서대로 왼쪽에서 오른쪽으로 읽힌다.

## [2026-07-26] site | Use colons for displayed document subtitles

변경 내용:

- 공개 문서·보존 자료의 H1, 디렉터리·홈·번역·관계·이전/다음 카드, breadcrumb, 빠른 검색·전체 검색·관계 탐색기에서 보이는 문서 제목의 공백으로 둘러싼 em dash(`—`)를 콜론(`:`)으로 표시한다. `같은 벡터 공간은 무엇을 보장하는가 — 공기·대조·생성의 경계`는 화면에서 `같은 벡터 공간은 무엇을 보장하는가: 공기·대조·생성의 경계`로 읽힌다.
- 정적 HTML과 JavaScript가 만드는 제목이 같은 표시 규칙을 쓰도록 했다. 원본 Markdown, 브라우저 탭 제목, ARIA 레이블, 정렬·검색 데이터와 본문 H2–H4는 원문 텍스트를 유지한다.

검증 결과:

- Node 회귀 152건, 사이트 빌드·검사, Playwright Chromium 16건과 `git diff --check`를 통과했다. 브라우저 검사는 H1·breadcrumb·디렉터리 카드·빠른/전체 검색·관계 탐색기의 콜론 표기와 H2의 비변경을 함께 확인한다.
- desktop과 390×844 viewport에서 문서 화면과 카드 목록의 제목 표기, 가로 overflow가 없음을 시각 검수했다.
