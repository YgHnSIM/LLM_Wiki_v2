...
[[데이터 희소성]] 때문에 한 토큰의 확률이 0이 되면 $\ln 0$은 유한한 수가 아니어서 perplexity가 무한대로 발산한다. [[Smoothing|평활화]]는 미관측 조합에 작은 확률을 배분해 이 극단을 피하려는 이유 중 하나다.
...
이 문서는 token별 평균과 평가 비교 조건을 맡는다. 로그의 정의역과 역함수 성질은 [[지수와 로그]]에서, 평균 NLL이 교차 엔트로피·KL과 어떻게 연결되는지는 [[엔트로피·교차 엔트로피·KL 발산]]에서 완전하게 다룬다.
