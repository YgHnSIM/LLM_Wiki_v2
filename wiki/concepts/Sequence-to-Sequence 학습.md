...
인코더가 가변 길이 입력을 내부 상태로 바꾸면 디코더가 그 상태와 앞서 나온 목표 요소를 조건으로 다음 요소를 하나씩 만든다. 종료 기호가 나오면 멈추므로 입력과 출력의 길이가 달라도 된다.
...
종단간 학습은 입력에서 목표 토큰 손실까지 매개변수를 함께 최적화한다는 뜻이지 말뭉치 구축·토큰화·어휘·탐색까지 인간의 선택이 사라진다는 뜻은 아니다. 번역 구조가 어텐션과 Transformer로 이어졌어도 모든 자기회귀 언어 모델·GPT·BERT의 발상을 seq2seq 하나로 환원할 수 없다.

## 학습 확인

1. Sequence-to-Sequence 학습은 입력과 출력의 길이가 달라도 되도록 어떤 두 구성요소를 결합하는가?
