...
> [!note] 학습 안내
> **난이도:** 중급<br>
> **선수 지식:** [[N-gram 모델]], [[데이터 희소성]]<br>
> **읽고 나면:** 평활화가 관측 확률에서 질량을 확보해 미관측 사건에 배분하는 이유와 백오프·보간의 차이를 설명할 수 있다.
...
평활화는 관측 빈도로 만든 확률을 그대로 확정하지 않는다. 관측 사건의 일부 확률 질량을 조정해 남기고, 정해진 규칙에 따라 보지 못한 사건에 나눠 전체 분포의 합을 1로 유지한다.
...
1. 평활화가 미관측 n-gram에 0이 아닌 확률을 주려면 관측 확률을 왜 조정해야 하는가?
2. Katz back-off는 관측 빈도 0과 낮은 양의 빈도를 어떻게 다르게 처리하는가?
3. 백오프 계수 `α(h)`와 보간 가중치 `λ`를 같은 값으로 볼 수 없는 이유는 무엇인가?

다음에는 [[019_Katz 백오프와 희소 데이터 확률 추정]]에서 네 분기와 정규화식을 자세히 본다. 확률 추정이 표현 계보에서 어떻게 달라졌는지 보려면 [[N-gram에서 LLM으로]] 이어 간다.
