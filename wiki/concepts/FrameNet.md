...
이 절차는 데이터에서 프레임이 자동으로 귀납됐다는 뜻이 아니다. FrameNet은 사람의 의미 분석을 말뭉치 증거로 제약하고, 검색·표집·주석 도구로 반복 가능한 편찬 과정에 넣은 자원이다. 사람이 만든 표현과 말뭉치에서 관찰한 실현을 결합한다는 점에서 [[규칙 기반 AI에서 데이터 기반 학습으로]]의 단순한 양자택일을 벗어나는 사례다.
