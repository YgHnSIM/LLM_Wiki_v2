...
> **난이도:** 입문<br>
> **선수 지식:** 없음<br>
> **읽고 나면:** ELIZA가 스크립트의 패턴과 재조립 규칙으로 대화를 이어 간 방식과 그 평가 한계를 설명할 수 있다.
...
자연스러운 대화 출력은 ELIZA가 문장의 의미나 세계 지식을 일반적으로 모델링했다는 증거가 아니다. 또한 현재 근거로는 ELIZA를 튜링 테스트 구현을 목적으로 한 최초 프로그램이라고 단정할 수 없다.

## 학습 확인

### 확인 질문

1. ELIZA는 하나의 치료 프로그램인가, 여러 스크립트를 실행할 수 있는 프레임워크인가?
2. 키워드 선택에서 재조립과 대화 복구까지의 응답 흐름은 어떻게 이어지는가?
3. ELIZA의 자연스러운 출력이 일반적인 의미 이해를 증명하지 않는 이유는 무엇인가?
