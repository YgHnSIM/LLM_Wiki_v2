...
생성 코드는 신뢰되지 않은 프로그램이다. 연구진은 gVisor와 network firewall을 포함한 sandbox에서 평가했고, 배포 환경에서는 자격을 갖춘 사람이 실행 전 결과를 검토해야 한다고 경고했다. Unit test 통과는 악성 동작, 취약한 API 사용과 숨은 논리 오류가 없다는 보증이 아니다.
