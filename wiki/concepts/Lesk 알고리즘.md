...
`cone` 주변에 `pine`과 나무 관련 단어가 있고 그 뜻풀이에도 같은 어휘가 겹친다면, 아이스크림 용기 의미보다 솔방울 의미에 더 높은 점수를 주는 식이다. 핵심은 뜻풀이의 문자열 중첩이지, 학습된 의미 벡터나 사람 수준의 상식 추론이 아니다.
...
1. Lesk 알고리즘은 어떤 두 종류의 텍스트에서 어휘 중첩을 찾는가?
2. 1986년 구현과 후대의 단순화 레스크는 비교 대상과 처리 방식이 어떻게 다른가?
3. 글로스가 짧거나 같은 문자열을 공유하지 않을 때 왜 판단이 어려워지는가?
