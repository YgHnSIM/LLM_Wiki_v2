...
소스 110개, 참고 자료 0개, 개념 187개, 개체 29개, 분석 27개와 메타 문서 3개, 총 356개 Markdown 문서가 있다. 전체 문서는 스키마 v2를 따르며 319개는 `verified`, 해석적 문서 34개는 `partial`, 철학적 결론이 논쟁 중인 문서 3개는 `disputed`다. 452개 외부 근거와 220개 불변 raw artifact가 레지스트리에 등록돼 있다.

[[지수와 로그]]와 [[엔트로피·교차 엔트로피·KL 발산]]은 확률·조건부확률에서 softmax·로그가능도·Perplexity로 이어지는 계산 경로를 보강한다. 두 문서는 각각 양수화·곱→합 변환과 한 분포의 평균 정보량·실제/모델 분포의 추가 로그 벌점을 맡는다. `LLM을 만든 수학` 허브는 미분·최적화의 다음 owner 배치가 준비될 때까지 공개하지 않는다.
