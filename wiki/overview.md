...
모델 밖의 시스템 경계를 따라 읽으려면 [[LLM 추론 에너지 지표]]에서 시작해 [[전력에서 서비스 결과 계약까지 무엇을 세어야 하나]]로 간다. 이 경로는 전력계의 J를 token 수 하나로 환산하지 않고, prefill·decode·idle·retry와 품질·deadline·traffic을 포함한 서비스 결과 계약으로 확장한다. 데이터·메모리·문자·권한·실시간 상호작용의 후속 경로는 별도 근거와 owner를 갖춘 뒤 같은 지도에 연결한다.
...
처음부터 모든 문서를 순서대로 읽을 필요는 없다. source 110개, reference 0개, concept 201개, entity 29개, analysis 42개를 합친 비메타 문서 382개는 모두 쉬운 핵심, 작동 원리, 기술과 근거의 세 단계로 구성됐다. 모르는 수식이 나오면 1단계와 2단계만 읽고 관련 개념으로 이동한 뒤 돌아와도 된다. 핵심 수학 경로는 완전 풀이를 읽는 데서 끝내지 않고 `부분 완성 → 새 수치 전이 → 오류 진단 → 해설과 채점 기준`으로 도움을 줄여 독립 계산을 확인하는 방향으로 보강 중이다.
...
- **확률 기호가 처음이면:** $\lvert A\rvert$는 사건 안의 결과 수, $\lvert\Omega\rvert$는 전체 결과 수, $P(A\mid B)$는 “$B$가 주어졌을 때 $A$”라고 먼저 소리 내어 읽는다. 이어서 [[확률]]의 카드 수 세기와 [[조건부 확률]]의 후보 좁히기 예를 따라간다.
- **[[LLM을 만든 수학]]이 처음이면:** 먼저 ‘계산 이야기’의 화살표를 읽고, 다섯 단계의 **입력·질문·출력** 문장만 따라간다. 그 뒤 필요한 식에서만 숫자와 shape를 확인하면 token에서 확률·오차·한 번의 갱신으로 이어지는 흐름을 놓치지 않는다.
- **읽은 내용을 실제로 해낼 수 있는지 확인하려면:** 핵심 concept와 허브의 `마스터리 연습`에서 본문과 다른 숫자를 풀고, 답을 본 뒤 같은 문제를 되풀이하지 말고 지정된 재시도 값으로 다시 계산한다. 전체 80% 이상이어도 확률 축·causal mask·shape·gradient 부호·분기 합산·head 분할 오류가 있으면 해당 owner를 복습한다.
- **준전문가 경계에 도전하려면:** owner별 마스터리 뒤 [[LLM 수학 종합 실습]]을 해설 없이 풀고, 20점 중 17점과 치명적 오류 0개를 동시에 만족한다. 실패한 행은 owner 경로로 돌아간 뒤 target과 실제 shape를 바꾼 전이 문제부터 다시 푼다.
