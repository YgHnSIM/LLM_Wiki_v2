...
처음부터 모든 문서를 순서대로 읽을 필요는 없다. source 110개, reference 0개, concept 185개, entity 29개, analysis 27개를 합친 비메타 문서 351개는 모두 쉬운 핵심, 작동 원리, 기술과 근거의 세 단계로 구성됐다. 모르는 수식이 나오면 1단계와 2단계만 읽고 관련 개념으로 이동한 뒤 돌아와도 된다. 특히 수식이 있는 문서는 선수 개념·가장 작은 계산 예·기호의 종류·가정·필연적 조건과 설계 선택을 분리해 설명하는 방향으로 보강 중이다.
...
소스 110개, 참고 자료 0개, 개념 189개, 개체 29개, 분석 28개와 메타 문서 3개, 총 359개 Markdown 문서가 있다. 전체 문서는 스키마 v2를 따르며 321개는 `verified`, 해석적 문서 35개는 `partial`, 철학적 결론이 논쟁 중인 문서 3개는 `disputed`다. 452개 외부 근거와 220개 불변 raw artifact가 레지스트리에 등록돼 있다.

[[LLM을 만든 수학]]은 4-token·2차원 toy causal attention에서 ID lookup, attention·잔차, 4개 logit의 softmax·NLL, 출력 bias 하나의 역전파와 SGD 갱신을 한 계산으로 연결한다. [[미분·편미분·그래디언트]]와 [[연쇄 법칙과 계산 그래프]]는 변화율·국소 방향·경로 합산을, [[경사하강법]]·[[역전파]]는 갱신·역모드 계산을 맡는다. 이 허브는 실제 LLM의 LayerNorm·FFN·다층·batch·Adam·분산 실행을 복제하지 않으며, 다음 통계·안정성 배치가 그 구분을 보완한다.
