...
언어 모델 발전과 인간 계산·stored program·GPU·분산 학습·memory·serving의 역사를 함께 읽으려면 [[LLM과 컴퓨팅 능력의 공진화]]에서 시작한다. 시대순 아홉 본편과 종합편이 “빨라졌다”를 계산 가능성·실현 성능·확장성·효율·신뢰성으로 나누어 안내한다.
...
| 언어 모델은 어떻게 발전했나 | [[확률]] | [[조건부 확률]] → [[로그가능도]] → [[001_섀넌의 N-gram 모델]] → [[N-gram 모델]] → [[마르코프 가정]] → [[데이터 희소성]] → [[Smoothing]] → [[019_Katz 백오프와 희소 데이터 확률 추정]] → [[Perplexity]] → [[단어 임베딩]] → [[다층 퍼셉트론]] → [[소프트맥스]] → [[신경 확률 언어 모형]] → [[035_신경 확률 언어 모형과 분산 단어 표현]] → [[자기회귀 생성]] | [[N-gram에서 LLM으로]] |
