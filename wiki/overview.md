...
소스 7개, 개념 49개, 개체 15개, 분석 7개와 메타 문서 3개, 총 81개 Markdown 문서가 있다. 전체 문서는 스키마 v2로 마이그레이션됐으며 69개는 `verified`, 해석적 분석과 논쟁적 개념 12개는 `partial`이다. 26개 외부 근거와 14개 불변 raw artifact가 레지스트리에 등록돼 있다.

`006`의 잘못된 MADALINE 약어·선형 LMS 국소 최솟값·VAD 배치 서술과 `007`의 튜링 테스트 직접 계보 주장은 위키 본문에서 교정했다. raw의 원문 결손과 오류는 보존하되 공개 지식의 기준으로 사용하지 않는다.
