...
언어 모델 발전과 인간 계산·stored program·GPU·분산 학습·memory·serving의 역사를 함께 읽으려면 [[LLM과 컴퓨팅 능력의 공진화]]에서 시작한다. 시대순 아홉 본편과 종합편이 “빨라졌다”를 계산 가능성·실현 성능·확장성·효율·신뢰성으로 나누어 안내하며, [[N-gram에서 LLM으로]]은 본편 4–6 사이의 희소 계수 → 학습 표현 → 조건부 생성 전환을 가로질러 잇는다.
...
처음부터 모든 문서를 순서대로 읽을 필요는 없다. source 110개, reference 0개, concept 200개, entity 29개, analysis 40개를 합친 비메타 문서 379개는 모두 쉬운 핵심, 작동 원리, 기술과 근거의 세 단계로 구성됐다. 모르는 수식이 나오면 1단계와 2단계만 읽고 관련 개념으로 이동한 뒤 돌아와도 된다. 핵심 수학 경로는 완전 풀이를 읽는 데서 끝내지 않고 `부분 완성 → 새 수치 전이 → 오류 진단 → 해설과 채점 기준`으로 도움을 줄여 독립 계산을 확인하는 방향으로 보강 중이다.

| 관심 | 권장 시작 | 이어 읽기 | 도착점 |
| --- | --- | --- | --- |
| LLM과 컴퓨팅 능력은 어떻게 함께 발전했나 | [[LLM과 컴퓨팅 능력의 공진화]] | 기계 이전 계산 → 형식 절차 → 초기 학습 → 통계 모델 → GPU → Transformer → 규모화 → memory → service | [[LLM 능력은 모델의 속성인가 시스템의 속성인가]]에서 능력층·측정 장부·인과를 종합하기 |
...
| 좋은 모델을 실제 서비스 능력으로 어떻게 바꾸나 | [[언어 모델 추론 서빙]] | [[훈련 병렬성과 생성 순차성은 다른 축이다]] → [[090_공개 가중치 LLM 파동과 서로 다른 공개 범위]] → [[091_QLoRA와 4비트 양자화 미세조정]] → [[097_Mixtral의 생산 배포 효율 주장과 증거 경계]] | [[모델 능력에서 서비스 능력으로]] |
