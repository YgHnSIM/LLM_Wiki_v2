...
처음부터 모든 문서를 순서대로 읽을 필요는 없다. source 110개, reference 0개, concept 183개, entity 29개, analysis 27개를 합친 비메타 문서 349개는 모두 쉬운 핵심, 작동 원리, 기술과 근거의 세 단계로 구성됐다. 모르는 수식이 나오면 1단계와 2단계만 읽고 관련 개념으로 이동한 뒤 돌아와도 된다. 특히 수식이 있는 문서는 선수 개념·가장 작은 계산 예·기호의 종류·가정·필연적 조건과 설계 선택을 분리해 설명하는 방향으로 보강 중이다.
...
| 현대 Transformer 계열은 어디서 왔나 | [[018_역전파와 다층 신경망 학습]] | [[다층 퍼셉트론]] → [[소프트맥스]] → [[045_Sequence-to-Sequence 학습과 신경 기계 번역]] → [[047_어텐션 메커니즘과 동적 정렬]] → [[어텐션 메커니즘]] → [[잔차 연결]] → [[Transformer]] → [[055_Transformer와 자기어텐션 기반 시퀀스 모델링]] → [[064_Transformer-XL과 세그먼트 수준 재귀]] → [[061_XLNet·RoBERTa·ALBERT의 BERT 개선 경로]] | [[066_신경 언어 모델의 스케일링 법칙]] |
...
소스 110개, 참고 자료 0개, 개념 183개, 개체 29개, 분석 27개와 메타 문서 3개, 총 352개 Markdown 문서가 있다. 전체 문서는 스키마 v2를 따르며 315개는 `verified`, 해석적 문서 34개는 `partial`, 철학적 결론이 논쟁 중인 문서 3개는 `disputed`다. 451개 외부 근거와 220개 불변 raw artifact가 레지스트리에 등록돼 있다.
