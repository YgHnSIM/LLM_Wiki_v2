...
소스 110개, 참고 자료 0개, 개념 189개, 개체 29개, 분석 27개와 메타 문서 3개, 총 358개 Markdown 문서가 있다. 전체 문서는 스키마 v2를 따르며 321개는 `verified`, 해석적 문서 34개는 `partial`, 철학적 결론이 논쟁 중인 문서 3개는 `disputed`다. 452개 외부 근거와 220개 불변 raw artifact가 레지스트리에 등록돼 있다.

[[미분·편미분·그래디언트]]는 변화율·편미분·그래디언트의 정의와 국소 방향 해석을, [[연쇄 법칙과 계산 그래프]]는 합성 경로의 곱과 재사용 노드의 기울기 합산을 맡는다. [[경사하강법]]·[[역전파]]는 각각 갱신과 역모드 계산의 책임을 이 기준 문서에 연결했고, [[다층 퍼셉트론]]·[[Transformer]]·[[순환 신경망]]·[[대규모 언어 모델]]에는 필요한 현재 수식과 shape만 남겼다. `LLM을 만든 수학` 허브는 이 배치의 전체 검증이 끝난 뒤 다음 배치에서 공개한다.
