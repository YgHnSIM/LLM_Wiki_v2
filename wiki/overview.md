...
처음부터 모든 문서를 순서대로 읽을 필요는 없다. source 110개, reference 0개, concept 182개, entity 29개, analysis 23개를 합친 비메타 문서 344개는 모두 쉬운 핵심, 작동 원리, 기술과 근거의 세 단계로 구성됐다. 모르는 수식이 나오면 1단계와 2단계만 읽고 관련 개념으로 이동한 뒤 돌아와도 된다. 특히 수식이 있는 문서는 선수 개념·가장 작은 계산 예·기호의 종류·가정·필연적 조건과 설계 선택을 분리해 설명하는 방향으로 보강 중이다.

| 관심 | 권장 시작 | 이어 읽기 | 도착점 |
| --- | --- | --- | --- |
| 언어 모델은 어떻게 발전했나 | [[확률]] | [[조건부 확률]] → [[001_섀넌의 N-gram 모델]] → [[N-gram 모델]] → [[마르코프 가정]] → [[데이터 희소성]] → [[Smoothing]] → [[019_Katz 백오프와 희소 데이터 확률 추정]] → [[Perplexity]] → [[단어 임베딩]] → [[다층 퍼셉트론]] → [[소프트맥스]] → [[신경 확률 언어 모형]] → [[035_신경 확률 언어 모형과 분산 단어 표현]] → [[자기회귀 생성]] | [[N-gram에서 LLM으로]] |
