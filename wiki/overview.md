...
- **읽은 내용을 실제로 해낼 수 있는지 확인하려면:** 핵심 concept와 허브의 `마스터리 연습`에서 본문과 다른 숫자를 풀고, 답을 본 뒤 같은 문제를 되풀이하지 말고 지정된 재시도 값으로 다시 계산한다. 전체 80% 이상이어도 확률 축·causal mask·shape·gradient 부호·분기 합산 오류가 있으면 해당 owner를 복습한다.
