...
완료 기준이 있는 입문 과정은 [[LLM 학습 가이드]]가 맡는다. 그 가이드는 짧은 진단 뒤 `공통 코어 → 주 전공 → 두 교차 과제 → 종합 감사`를 제안하고, 각 단위에서 읽기만이 아니라 산출물·통과 기준·오류별 복귀를 제공한다. 이 문서는 그 과정을 다시 나열하지 않는다.

아래 표는 이미 관심 질문이 있거나 특정 문서를 찾아야 할 때 쓰는 **고급 탐색 지도**다. source 110개, reference 0개, concept 204개, entity 29개, analysis 45개를 합친 비메타 문서 388개는 모두 쉬운 핵심, 작동 원리, 기술과 근거의 세 단계로 구성됐다. 모르는 수식이 나오면 현재 문서의 1·2단계와 필요한 concept를 왕복해도 된다. 표의 한 행을 모두 끝낼 필요도 없으며, 각 경로가 가리키는 질문과 도착점이 지금의 목적에 맞는지만 먼저 확인한다.

| 관심 | 권장 시작 | 이어 읽기 | 도착점 |
| --- | --- | --- | --- |
| LLM과 컴퓨팅 능력은 어떻게 함께 발전했나 | [[LLM과 컴퓨팅 능력의 공진화]] | 아홉 본편의 시대순 경로 + 표현·software·silicon의 세 횡단 연결고리 | [[LLM 능력은 모델의 속성인가 시스템의 속성인가]]에서 능력층·측정 장부·인과를 종합하기 |
| 기계 이전에도 계산 능력은 어떻게 확장됐나 | [[기계 이전의 계산은 어떻게 능력이 되었나]] | [[001_섀넌의 N-gram 모델]] → [[N-gram 모델]] → [[단어 임베딩]] | 선계산·분업·검산과 학습된 parameter를 직접 계보 없이 비교하기 |
| 기호는 어떻게 실행 가능한 절차와 확률 언어가 됐나 | [[언어와 계산을 기계적 절차로 만들다]] | [[앨런 튜링]] → [[클로드 섀넌]] → [[001_섀넌의 N-gram 모델]] → [[계산 복잡도와 비용 모델]] | 논리식·instruction·program·parameter·확률분포의 역할을 구분하기 |
| 저장된 프로그램은 어떻게 학습 실행 환경이 됐나 | [[저장 프로그램에서 학습 프레임워크까지]] | Stored instruction → Fortran compiler → BLAS → [[연쇄 법칙과 계산 그래프]] → CUDA·cuDNN → TensorFlow | 의미·shape·수치·성능 계약과 compiler·library·autodiff·runtime의 책임 구분하기 |
| 더 많은 transistor는 어떻게 AI 가속기가 됐나 | [[집적도 증가에서 가속기까지 무엇이 더 필요했나]] | Moore 1965 → Dennard scaling → power·memory wall → parallel shift → CUDA GPU → TPU | Component density·peak arithmetic·kernel·service latency를 분리하고 단일 인과 피하기 |
| 학습 규칙만으로 왜 큰 학습 시스템이 되지 않나 | [[학습 규칙이 있어도 왜 규모화되지 못했나]] | [[003_Georgetown-IBM 기계 번역 시연]] → [[퍼셉트론]] → [[ADALINE]] → [[LMS 알고리즘]] → [[퍼셉트론에서 MADALINE으로]] | Rule 실행과 parameter 학습, representation·credit·compute·평가 병목을 구분하기 |
| 확률 언어 모델은 어떤 계산 인프라에서 커졌나 | [[확률적 언어 모델은 어떤 계산 인프라를 요구했나]] | [[001_섀넌의 N-gram 모델]] → [[N-gram 모델]] → [[013_은닉 마르코프 모델과 통계적 음성 인식]] → [[019_Katz 백오프와 희소 데이터 확률 추정]] → [[최고 경로와 기대 통계, 백오프]] | Corpus·model·training·search·serving 규모와 metric을 별도 장부로 기록하기 |
| 행렬곱 가속은 어떻게 큰 딥러닝 실험을 현실화했나 | [[행렬곱 가속은 딥러닝을 어떻게 현실화했나]] | [[018_역전파와 다층 신경망 학습]] → [[021_합성곱 신경망과 특징 학습]] → [[가속기와 행렬 계산]] → [[메모리 계층과 데이터 이동]] | GPU·CUDA·cuDNN·framework의 역할과 peak·kernel·step·time-to-quality를 구분하기 |
| 언어 모델은 어떻게 발전했나 | [[확률]] | [[조건부 확률]] → [[로그가능도]] → [[001_섀넌의 N-gram 모델]] → [[N-gram 모델]] → [[마르코프 가정]] → [[데이터 희소성]] → [[Smoothing]] → [[019_Katz 백오프와 희소 데이터 확률 추정]] → [[신경 확률 언어 모형]] → [[043_Word2Vec와 효율적 정적 단어 임베딩]] → [[045_Sequence-to-Sequence 학습과 신경 기계 번역]] → [[Transformer]] | [[N-gram에서 LLM으로]]에서 과업·표현·실행 계약의 변화를 종합하기 |
| LLM의 표현과 변환은 어떤 수학을 쓰나 | [[벡터·행렬·텐서와 shape]] | [[내적·행렬곱과 선형변환]] → [[단어 임베딩]] → [[어텐션 메커니즘]] → [[잔차 연결]] → [[Transformer]] | 새 ID·shape·mask에서도 lookup·가중합·residual을 독립 계산 |
| 실제 Transformer tensor 축은 어떻게 이어지나 | [[Transformer]] | [[확률변수·확률분포·기대값·분산]] → [[Layer Normalization]] → [[활성화 함수]] → [[LLM을 만든 수학]]의 shape 전이 → `npm run math:shapes` | batch·head·query/key·FFN·vocab 축과 gradient shape 추적 |
| LLM은 손실로 가중치를 어떻게 배우나 | [[미분·편미분·그래디언트]] | [[연쇄 법칙과 계산 그래프]] → [[역전파]] → [[경사하강법]] → [[다층 퍼셉트론]] → [[Transformer]] → [[대규모 언어 모델]] | 새 함수·분기·행렬에서도 gradient를 계산하고 실제 갱신과 구분 |
| 다음 token 출력단을 새 숫자로 계산할 수 있나 | [[소프트맥스]] | [[로그가능도]] → [[경사하강법]] → [[LLM을 만든 수학]]의 마스터리 연습 | logit·확률·NLL·gradient·update를 독립 계산하고 축·부호 오류 진단 |
| 역전파 구현이 수식과 같은지 어떻게 점검하나 | [[연쇄 법칙과 계산 그래프]] | [[역전파]] → [[LLM을 만든 수학]]의 전체 출력층 gradient → `npm run math:toy` | $W,b,r$ gradient의 shape·경로 합과 중심 차분 일치 확인 |
| 단원별 계산을 새 문제에서 끝까지 연결할 수 있나 | [[LLM 수학 종합 실습]] | 무도움 풀이 → `npm run math:capstone` 검산 → 오류별 owner 재학습 | lookup부터 gradient·실제 shape까지 20점 rubric과 치명적 오류 0개 |
...
| Transformer의 병렬성은 장치에서 무엇을 뜻하나 | [[가속기와 행렬 계산]] | [[메모리 계층과 데이터 이동]] → [[055_Transformer와 자기어텐션 기반 시퀀스 모델링]] → [[Transformer는 무엇을 병렬화했고 무엇을 남겼나]] | 훈련 위치 병렬성·생성 순차성·HBM I/O를 같은 속도 주장으로 합치지 않기 |
| 모델·데이터·장치를 얼마나 키울지 어떻게 정하나 | [[언어 모델 스케일링 법칙]] | [[066_신경 언어 모델의 스케일링 법칙]] → [[078_Chinchilla와 계산 최적 언어 모델 학습]] → [[분산 학습과 집단 통신]] → [[082_BLOOM과 공개 접근 다국어 LLM]] → [[083_PaLM과 Pathways 기반 대규모 언어 모델 확장]] | [[규모는 언제 연구 변수가 되었나]] |
| FLOPs보다 데이터 이동이 병목일 때 무엇을 바꾸나 | [[메모리 계층과 데이터 이동]] | [[수치 형식·혼합 정밀도·양자화]] → [[088_FlashAttention과 IO 인지형 정확 어텐션]] → [[069_전문가 혼합과 희소 활성 스케일링]] → [[091_QLoRA와 4비트 양자화 미세조정]] | [[연산보다 데이터 이동이 비싸질 때]] |
| 좋은 모델을 실제 서비스 능력으로 어떻게 바꾸나 | [[언어 모델 추론 서빙]] | [[LLM 추론 에너지 지표]] → [[전력에서 서비스 결과 계약까지 무엇을 세어야 하나]] → [[훈련 병렬성과 생성 순차성은 다른 축이다]] → [[090_공개 가중치 LLM 파동과 서로 다른 공개 범위]] → [[091_QLoRA와 4비트 양자화 미세조정]] → [[097_Mixtral의 생산 배포 효율 주장과 증거 경계]] | [[모델 능력에서 서비스 능력으로]] |
...
| 모델의 제안은 언제 실제 행동이 되나 | [[080_사고 연쇄 프롬프팅과 추론 행동 유도]] | [[092_함수 호출과 도구 사용의 모델-실행 경계]] → [[문자에서 실행 권한까지]] → [[함수 호출과 도구 사용]] → [[104_LLM 에이전트의 추론-행동 루프와 자율성 경계]] → [[LLM 에이전트]] → [[109_AI 공동 과학자의 가설 생성과 자율 연구 경계]] | [[자동 평가 지표는 무엇을 보상하는가]] |
...
- **처음부터 준전문가 경계까지:** [[LLM 학습 가이드]]에서 진단·산출물·통과 기준·복귀 경로를 사용한다. 이 경로는 방문 수나 읽은 시간으로 완료를 판정하지 않는다.
- **계산이 궁금할 때:** [[LLM을 만든 수학]]은 token·shape·확률·손실·gradient를 하나의 toy 계산으로 잇고, [[LLM 수학 종합 실습]]은 새 수치에서 20점 중 17점·치명적 오류 0개 기준을 확인한다.
- **역사와 근거가 궁금할 때:** [[LLM과 컴퓨팅 능력의 공진화]]는 시대별 능력 조건과 횡단 연결고리를, [[N-gram에서 LLM으로]]은 표현·과업·실행 계약의 변화를 비교한다.
- **모델 밖의 결과가 궁금할 때:** [[LLM 시스템 경계 확장 지도]]는 에너지·data·memory·문자·외부 효과·실시간 상호작용의 owner를 질문별로 고르게 하고, [[LLM 능력은 모델의 속성인가 시스템의 속성인가]]은 model과 service 결과를 같은 능력으로 부르지 않게 한다.

표에 없는 특정 페이지를 유형별로 찾으려면 [[index]]를, 최근 변경과 남은 제한을 확인하려면 [[log]]를 사용한다.
...
- [[104_LLM 에이전트의 추론-행동 루프와 자율성 경계]]는 ReAct·Toolformer·Reflexion·Generative Agents의 2022–2023년 선행 계보와 2024년 WebArena·SWE-bench·OSWorld·ToolEmu의 평가 범위를 복원하고, model·orchestrator·tool·state·policy를 분리해 자율성·memory·학습·안전·응용 성과의 과장을 교정한다.
...
- [[110_저자원 언어 LLM의 성능 격차와 전이·평가 경계]]는 이름 붙이지 않은 2025년 단일 돌파 서사를 Alhanai·Nagar·de Gibert의 서로 다른 아프리카 언어 평가·힌디어 코드스위칭·합성 기계번역 실험으로 나누고, XLM·NLLB·BLOOM·MaLA·Aya의 선행 경로와 data·tokenizer·전이·평가·배포의 별도 장부를 복원한다. 벤치마크 개선을 여러 언어의 영어 동등성이나 교육·의료·경제·문화 보존의 실제 효과로 확대하지 않는다.

## 현재 핵심 주제

- [[확률]], [[조건부 확률]], [[N-gram 모델]], [[데이터 희소성]], [[Smoothing]], [[Perplexity]]
...
- [[자동 평가 지표는 무엇을 보상하는가]]에서 비교하는 BLEU·ROUGE·METEOR의 분모·대응 단위, MMLU 문항 가중·과목 macro와 BIG-bench low/high 정규화 task 평균의 투표권·환율, GPT-4 UBE raw score→scaled score→비교 모집단→백분위의 변환 경계, HumanEval 실행 검사, InstructGPT·RL-CAI 선호 보상과 HELM 시나리오×메트릭 장부 및 각 단계에 남는 프록시·가중치·가치 판단
...
- [[105_통합 멀티모달 아키텍처의 공유 범위와 입출력 경계]]와 [[멀티모달 대규모 언어 모델]]에서 구분하는 joint embedding·modular bridge·shared encoder-decoder·early-fusion token core, modality tokenizer·decoder 경계와 model I/O·제품 rollout
...
- [[멀티모달 대규모 언어 모델]]에서 구분하는 image encoder·bridge·언어 decoder·동결 경계·출력 signature, Frozen의 연속 prefix·Flamingo의 별도 cross-attention memory·PaLI의 encoder-decoder visual token·BLIP-2의 Q-Former·LLaVA의 projection·PaLM-E의 continuous observation token 및 GPT-4V의 비공개 architecture
- [[GPT-4]]에서 분리하는 base·post-training·image+text→text interface·ChatGPT/API 제품 snapshot, 3월 text 공개와 9월 vision rollout, 시험 protocol·UBE percentile 모집단, 조건부 안전 개선과 비공개 parameter·vision bridge·data·compute
...
- [[자동 평가 지표는 무엇을 보상하는가]]에서 비교하는 BLEU·ROUGE·METEOR의 참조 중첩, MMLU 문항·과목과 BIG-bench task·subset 집계, GPT-4 UBE의 점수→비교 모집단→백분위 환산, HumanEval unit test, HELM의 다차원 행렬, Function Calling의 JSON→schema→의미→권한→실행→grounding 관문, InstructGPT·RL-CAI 평가자 연쇄 및 분모·정규화·비교 집단·pass@k·피드백 출처·prompt 분포가 점수 유인에 들어가는 서로 다른 위치
...
[[LLM 수학 종합 실습]]은 허브와 다른 임베딩·attention·target으로 lookup부터 출력 gradient까지 다시 계산하게 하고, 20점 rubric과 치명적 오류 0개 조건으로 단원 간 전이를 판정한다. `npm run math:capstone`은 확률 $4/7$, NLL $\ln(7/4)$, 전체 출력 gradient, FFN residual 분기 합과 SGD 손실 감소를 검산하지만, 답안의 축·가정·한계 설명을 대신하지 않는다.
...
[[언어 수와 언어 형평성은 같은 축인가]]는 XLM의 XNLI 15개 언어, BLOOM의 46개 자연어, NLLB의 약 200개 번역 언어, MaLA-500의 534개 language-script 단위처럼 분모가 다른 count를 한 순위표에 놓지 않는다. 목록·입력, data, tokenizer, capacity·transfer, 평가, 접근·비용·안전, 공동체 권한·outcome의 일곱 관문으로 coverage 확대와 실제 분배 결과를 분리한다.
...
[[105_통합 멀티모달 아키텍처의 공유 범위와 입출력 경계]]는 ‘통합’을 하나의 구조로 취급하지 않는다. CLIP식 joint embedding, Flamingo식 bridge, Unified-IO 2의 shared encoder-decoder, Chameleon·Emu3의 early-fusion token core를 구분하고, GPT-4o 제품 interface의 동시 입력·출력과 공개된 학습·checkpoint 범위를 같은 주장으로 합치지 않는다.

[[104_LLM 에이전트의 추론-행동 루프와 자율성 경계]]와 [[LLM 에이전트]]는 model 제안과 실제 tool 실행 사이의 orchestrator·권한·상태·종료 조건을 분리한다. ReAct·Toolformer·Reflexion·Generative Agents의 서로 다른 memory·학습 위치와 WebArena·SWE-bench·OSWorld·ToolEmu의 초기 성공·안전 지표를 같은 자율성 점수로 합치지 않는다.

[[103_GLaM에서 Mixtral까지의 희소 MoE 확장]]과 [[전문가 혼합]]은 total parameters, token당 active parameters, training·serving memory와 장치 간 통신을 별도 장부로 둔다. GLaM과 Mixtral의 보고 수치를 공개 조건·hardware·batch·router 설정에 묶고, expert 명칭을 안정된 의미 domain이나 별도 추론 기관의 증거로 읽지 않는다.
...
[[사전 학습 지식은 과제에 어떻게 도착하는가]]에는 파운데이션 모델이 특정 adaptation algorithm이 아니라 기존 경로를 공통 기반–적응–배포 관계로 읽는 상위 범주라는 점을 기록했다. QLoRA를 더한 과제 적응 지도에서는 전체 weights·저순위 adapter·입력 context 가운데 task supervision이 놓이는 위치와 base storage dtype·compute dtype·trainable parameter를 분리한다. 멀티모달 이해 경로는 Frozen의 연속 prefix, Flamingo의 별도 visual memory, PaLI의 encoder–decoder visual token, BLIP-2의 Q-Former, LLaVA의 projection과 PaLM-E의 continuous observation token을 **표현 운반 방식**과 **어느 parameter를 갱신했는가**라는 두 축으로 비교한다. GPT-4V는 내부 architecture가 공개되지 않았으므로 같은 입출력만으로 어느 bridge를 사용했는지 추론하지 않는다. 생성기를 새로 대규모 학습한 DALL·E 2·Stable Diffusion은 별도의 사전 학습 자산 재사용 표로 유지하며, CLIP image-embedding target과 Stable Diffusion의 text-token condition·공간 latent를 멀티모달 이해 bridge와 합치지 않는다. Model-parameter gradient·continuous-input gradient·output-only API 접근권과 foundation checkpoint·adapted model·deployed system의 관측 층위도 그대로 분리했다. [[손실 곡선과 능력 곡선 사이]]에는 Bommasani 등의 2021년 emergence가 암묵적으로 유도된 행동이라는 넓은 연구 의제이고, Wei 등의 2022년 emergent ability는 작은 규모에 없다가 큰 규모에서 관찰되는 task-level 판정이라는 용어 차이를 기록했다.
