...
숫자 badge는 정규 source의 공식 장 번호에만 쓴다. 비번호 참고 자료인 [[GLaM에서 Mixtral까지의 희소 MoE 확장]]은 source 목록 끝에서 `참고`로 표시하며, 목록 위치를 근거로 `001` 같은 장 번호를 만들지 않는다. 같은 장의 검증 노트·번역·해설은 서로 다른 장이 아니라 하나의 source 묶음에 속한다.
